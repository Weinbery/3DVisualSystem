...
Sidebar.Geometry.TeapotBufferGeometry = function ( signals, object ) {
	...
	segmentsRow.add( new UI.Text( '片段' ).setWidth( '90px' ) );
	...
	lidRow.add( new UI.Text( '盖' ).setWidth( '90px' ) );
	lidRow.add( lid );
	...
	bodyRow.add( new UI.Text( '主体' ).setWidth( '90px' ) );
	bodyRow.add( body );
	...
	fitLidRow.add( new UI.Text( '安装盖子' ).setWidth( '90px' ) );
